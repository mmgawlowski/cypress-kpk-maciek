...
class FormPage {
    submitFormWithValidData() {
        ...
        cy.get(lname).clear().type('G.').then(() => {
            expect((Cypress.$(form)[0] as HTMLFormElement).checkValidity()).to.be.true;
    ...
    tryToSubmitFormWithInvalidData() {
        ...
        cy.get(form).should($form => expect(($form[0] as HTMLFormElement).checkValidity()).to.be.false);
        cy.get(fname).should($fname => expect(($fname[0] as HTMLInputElement).validationMessage).to.be.equal('Please fill out this field.'));
