...
class DatePickerPage {
    checkDatePicker(date: string) {
        ...
        if (dateToSet > maxDate) {
            ...
            cy.get(datePicker).should($datePicker => expect(($datePicker[0] as HTMLInputElement).validationMessage).to.be.equal('Value must be 31/12/2020 or earlier.'));
        } else if (dateToSet < minDate) {
            cy.get(datePicker).invoke('prop', 'validity').its('rangeUnderflow').should('be.true');
            cy.get(datePicker).should($datePicker => expect(($datePicker[0] as HTMLInputElement).validationMessage).to.be.equal('Value must be 01/01/2020 or later.'));
        } else {
            cy.get(datePicker).should($datePicker => expect(($datePicker[0] as HTMLInputElement).checkValidity(), 'valid value').to.be.true);
        }  
    }
